...
void procpkt(u_char *useless,const struct pcap_pkthdr* pkthdr,const u_char* pack){
	...
	if(pkt->SID==1 && pkt->direction=='E'){
		Q4.push(pkt);
	}
	else if(pkt->SID==2 && pkt->direction=='E'){
		Q5.push(pkt);
	}
	else if(pkt->SID==3 && pkt->direction=='E'){
		Q6.push(pkt);
	}
	else{
		if(int_node==0){
	...
	if(st->direction=='A' || st->direction=='B'){
		if(state&(1<<5)){
			if(fwd_int&4){
				...
				fprintf(fp,"Forwarding Packet from Q4\n");
				fflush(fp);
			}
			else{
				fprintf(fp,"Servicing Q4\n");
				fflush(fp);
			}				
			Q4.pop();
		}
		...
		if(PS==0 || PS==3){
			if(state&(1<<3)){
				if(fwd_int&1){
					pkt=Q6.front();
					result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
					fprintf(fp,"Forwarding Packet from Q6\n");
					fflush(fp);
				}
				else{
					fprintf(fp,"Sevicing Q6\n");
					fflush(fp);
				}
				Q6.pop();
			}
			if(state&(1<<4)){
				if(fwd_int&2){
					pkt=Q5.front();
					result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
					fprintf(fp,"Forwarding Packet from Q5\n");
					fflush(fp);
				}
				else{
					fprintf(fp,"Servicing Q5\n");
					fflush(fp);
				}
				Q5.pop();
			}
...
void *send_update_thread(void *args){
	...
	while(1){
		...
		pthread_mutex_lock(&m);
		fprintf(fp,"INJECTING EAST UPDATE:%d\n",st.state);
		fflush(fp);	
		pthread_mutex_unlock(&m);
