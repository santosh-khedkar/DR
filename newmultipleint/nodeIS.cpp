...
void procpkt(u_char *useless,const struct pcap_pkthdr* pkthdr,const u_char* pack){
	...
	if(pkt->SID==1 && pkt->direction=='S'){
		Q9.push(pkt);
	}
	else if(pkt->SID==2 && pkt->direction=='S'){
		Q8.push(pkt);
	}
	else if(pkt->SID==3 && pkt->direction=='S'){
		Q7.push(pkt);
	}
	else{
		if(int_node==0){
...
void Take_action(struct state_t *st){
	...
	if(st->direction=='C' || st->direction=='D'){
		if(state&1){
			if(fwd_int&1){
				pkt=Q7.front();
				result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
				fprintf(fp,"Forwarding Packet from Q7\n");
				fflush(fp);
			}
			else{
				fprintf(fp,"Servicing Q7\n");
				fflush(fp);
			}				
			Q7.pop();
		}
		...
		if(PS==0 || PS==1){
			...
			if(state&(1<<1)){
				if(fwd_int&2){
					pkt=Q8.front();
					result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
					fprintf(fp,"Forwarding Packet from Q8\n");
					fflush(fp);
				}
				else{
					fprintf(fp,"Servicing Q8\n");
					fflush(fp);
				}				
				Q8.pop();
			}
			if(state&(1<<2)){
				if(fwd_int&4){
					pkt=Q9.front();
					result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
					fprintf(fp,"Forwarding Packet from Q9\n");
					fflush(fp);
				}
				else{
					fprintf(fp,"Servicing Q9\n");
					fflush(fp);
				}				
				Q9.pop();
			}
...
void *send_update_thread(void *args){
	...
	while(1){
		...
		pthread_mutex_lock(&m);
		fprintf(fp,"INJECTING SOUTH UPDATE:%d\n",st.state);
		fflush(fp);	
		pthread_mutex_unlock(&m);
