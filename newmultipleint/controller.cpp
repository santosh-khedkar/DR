...
void* send_update_thread(void *args){
	...
	//-----------------------------------------------------------------------------------------------	
	/* Find the properties for the device */
	if (pcap_lookupnet(dev[3], &net1, &mask1, errbuf1) == -1) {
		fprintf(stderr, "Couldn't get netmask for device %s: %s\n", dev[3], errbuf1);
		net1 = 0;
		mask1 = 0;
	}
	...
	/* Open the session in promiscuous mode */
	handle1 = pcap_open_live(dev[3], BUFSIZ, 1, 1000, errbuf1);
	if (handle1 == NULL) {
		fprintf(stderr, "Couldn't open device %s: %s\n", dev[3], errbuf1);
		return (void*)2;
	}
	...
	//-----------------------------------------------------------------------------------------------	
	/* Find the properties for the device */
	if (pcap_lookupnet(dev[4], &net2, &mask2, errbuf2) == -1) {
		fprintf(stderr, "Couldn't get netmask for device %s: %s\n", dev[4], errbuf2);
		net2 = 0;
		mask2 = 0;
	}
	...
	/* Open the session in promiscuous mode */
	handle2 = pcap_open_live(dev[4], BUFSIZ, 1, 1000, errbuf2);
	if (handle2 == NULL) {
		fprintf(stderr, "Couldn't open device %s: %s\n", dev[4], errbuf2);
		return (void*)2;
	}
	...
	//-----------------------------------------------------------------------------------------------	
	/* Find the properties for the device */
	if (pcap_lookupnet(dev[5], &net3, &mask3, errbuf3) == -1) {
		fprintf(stderr, "Couldn't get netmask for device %s: %s\n", dev[5], errbuf3);
		net3 = 0;
		mask3 = 0;
	}
	...
	/* Open the session in promiscuous mode */
	handle3 = pcap_open_live(dev[5], BUFSIZ, 1, 1000, errbuf3);
	if (handle3 == NULL) {
		fprintf(stderr, "Couldn't open device %s: %s\n", dev[5], errbuf3);
		return (void*)2;
	}
	...
	//-----------------------------------------------------------------------------------------------	
	/* Find the properties for the device */
	if (pcap_lookupnet(dev[6], &net4, &mask4, errbuf4) == -1) {
		fprintf(stderr, "Couldn't get netmask for device %s: %s\n", dev[6], errbuf4);
		net4 = 0;
		mask4 = 0;
	}
	...
	/* Open the session in promiscuous mode */
	handle4 = pcap_open_live(dev[6], BUFSIZ, 1, 1000, errbuf4);
	if (handle4 == NULL) {
		fprintf(stderr, "Couldn't open device %s: %s\n", dev[6], errbuf4);
		return (void*)2;
	}
...
int main(int argc, char *argv[]){
	fp = fopen(argv[1],"w");
	fplog = fopen(argv[2],"w");
	pthread_t thread1,thread2,thread3,thread4,traffic_t,send_update;
	pthread_create(&thread1,NULL,sniffingthread,argv[3]);
	pthread_create(&thread2,NULL,sniffingthread,argv[4]);
	pthread_create(&thread3,NULL,sniffingthread,argv[5]);
	pthread_create(&thread4,NULL,sniffingthread,argv[6]);
