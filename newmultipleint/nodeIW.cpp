...
void procpkt(u_char *useless,const struct pcap_pkthdr* pkthdr,const u_char* pack){
	...
	if(pkt->SID==1 && pkt->direction=='W'){
		Q10.push(pkt);
	}
	else if(pkt->SID==2 && pkt->direction=='W'){
		Q11.push(pkt);
	}
	else if(pkt->SID==3 && pkt->direction=='W'){
		Q12.push(pkt);
	}
	else{
		if(int_node==0){
	...
	if(st->direction=='A' || st->direction=='B'){
		if(state&1){
			if(fwd_int&1){
				...
				fprintf(fp,"Forwarding Packet from Q10\n");
				fflush(fp);
			}
			else{
		...
		if(PS==0 || PS==1){
			...
			if(state&(1<<1)){
				if(fwd_int&2){
					pkt=Q11.front();
					result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
					fprintf(fp,"Forwarding Packet from Q11\n");
					fflush(fp);
				}
				else{
			...
			if(state&(1<<2)){
				if(fwd_int&4){
					pkt=Q12.front();
					result = pcap_inject(Fwd_handle, pkt,sizeof(struct packet));
					fprintf(fp,"Forwarding Packet from Q12\n");
					fflush(fp);
				}
				else{
